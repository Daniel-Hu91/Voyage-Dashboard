...
st.set_page_config(page_title="Lumpsum Calculator", layout="wide")
...
if "saved_lumpsum_results" not in st.session_state:
    st.session_state["saved_lumpsum_results"] = []

if "use_auto_wait_days_lumpsum" not in st.session_state:
    st.session_state["use_auto_wait_days_lumpsum"] = True

if "wait_days_lumpsum" not in st.session_state:
    st.session_state["wait_days_lumpsum"] = 0.0
...
with rev1:
    lumpsum_revenue = st.number_input("Lumpsum Revenue", min_value=0.0, value=0.0, step=10000.0)
with rev2:
    tonnage = st.number_input("Tonnage (for reference only)", min_value=0.0, value=35000.0, step=1000.0)

if st.session_state.get("use_auto_wait_days_lumpsum", True):
    if open_port != "Select" and ballast_port != "Select":
        try:
            ...
            if ballast_port == "Singapore" and auto_ballast_days < 14:
                st.session_state["wait_days_lumpsum"] = 14 - auto_ballast_days
            else:
                st.session_state["wait_days_lumpsum"] = 0.0
...
with exp5:
    wait_days = st.number_input(
        ...
        min_value=0.0,
        step=0.01,
        key="wait_days_lumpsum",
        disabled=st.session_state.get("use_auto_wait_days_lumpsum", True)
    )
    st.checkbox("Use Auto Wait Days", key="use_auto_wait_days_lumpsum")

bunker_spread = port_bunker_cost - bunker_cost

if st.button("Calculate Lumpsum TCE"):
    try:
        ...
        commission_costs = FIXED_ASSUMPTIONS["commission"] * lumpsum_revenue
        net_income = lumpsum_revenue - commission_costs

        total_sailing_days = ballast_days + laden_days
        wait_days = st.session_state.get("wait_days_lumpsum", 0.0)
        ...
        total_expenses = bunker_costs + port_costs + emissions_cost
        ...
        st.session_state["saved_lumpsum_results"].append(result)

    except Exception as e:
        st.exception(e)

if st.button("Clear Saved Lumpsum Calculations"):
    st.session_state["saved_lumpsum_results"] = []

st.subheader("Saved Lumpsum Calculations")

saved_results = st.session_state.get("saved_lumpsum_results", [])

if not saved_results:
    st.info("No saved lumpsum calculations yet.")
else:
