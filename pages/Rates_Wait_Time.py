...
st.set_page_config(page_title="Rates Comparison With Wait Time", layout="wide")
st.title("Rates Comparison With Wait Time")
st.write(
    "A wait time has been factored in prior to loading. For example, if a ship is ballasted from Sikka to Singapore, there will be a wait time of give days."
)
...
st.markdown(
    ...
    **Wait Time:** {FIXED_RATE_ASSUMPTIONS['load_wait_days']} |
    **Wait Time Fuel:** {FIXED_RATE_ASSUMPTIONS['wait_fuel_tpd']} |
    **Tonnage:** {FIXED_RATE_ASSUMPTIONS['tonnage']}
    """
)

tab1, tab2, tab3, tab4 = st.tabs(["Kwinana", "Botany Bay", "Sikka", "Dar es Salaam"])
...
def render_rate_comparison_tab(
    ...
    if st.button("Run Route Economics", key=f"{key_prefix}_route_economics"):
        try:
            ...
            load_wait_days = FIXED_RATE_ASSUMPTIONS["load_wait_days"]
            wait_fuel_tpd = FIXED_RATE_ASSUMPTIONS["wait_fuel_tpd"]
            ...
            wait_fuel_cost = load_wait_days * wait_fuel_tpd * port_fuel_price
            ...
            total_days = total_sailing_days + total_port_days + load_wait_days
    ...
    if route_result is not None:
        ...
        st.write(f"**Wait Fuel Costs:** ${route_result['wait_fuel_cost']:,.2f}")
